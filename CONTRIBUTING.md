...
tests/fm-teardown.test.sh                 # fm-teardown.sh landed-work safety and reminder checks: fork-remote allow, squash/content landings, dirty and unlanded refusals, PR-head metadata, tasks-axi reminder, --force override, descriptive-branch HEAD-driven safety
...
tests/fm-gh.test.sh                       # fm-gh.sh per-repo GitHub account routing: owner from -R/origin, config lookup, idempotent switch, unmapped/unresolved warn-but-run, gh-axi-preferred exec with gh fallback
tests/fm-branch-name.test.sh              # descriptive work-branch decoupling: slugify/resolve/meta-read lib, fm-brief + fm-spawn branch recording, review-diff/merge-local/promote honoring branch= with fm/<id> fallback
